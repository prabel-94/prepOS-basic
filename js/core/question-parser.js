...
export function normalizeQuestionBlockMarkers(text) {
  ...
  // ⭐ IMPROVED: Only convert bare numbers that look like question numbers
  // Pattern: number at line start, followed by question stem (not a statement)
  // We look for: number + text that ends with ? or has question keywords
  // OR number + blank line (statement section start)
  
  // First pass: mark likely question numbers that have question indicators
  normalized = normalized.replace(
    ...
    (match, num, content) => {
      const hasQuestionIndicator =
        ...
        /\bFollow|Consider the following/i.test(content);
...
function applyQcpDeepCleanLines(text) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (isQuestionNumber) {
      ...
      // ⭐ IMPROVED: Detect if this is truly a question
      const hasQuestionPattern =
        isBlockStart ||
        /[:?][""']?\s*$/.test(content) ||
        /\bConsider the following statements?\b/i.test(content) ||
        /\bWhich\b/i.test(content) ||
        /\bWhat\b/i.test(content) ||
        /\bWho\b/i.test(content) ||
        /\bWhy\b/i.test(content) ||
      ...
      if (hasQuestionPattern) {
        ...
        // ⭐ NEW: If it doesn't look like a question, DON'T convert it
        // This preserves numbered statements (1., 2., 3., etc.) within questions
