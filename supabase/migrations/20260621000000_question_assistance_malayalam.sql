-- Persist Malayalam assistance mask on the same question row (question_metadata).
-- Display-only overlay; English content continues to drive question_hash dedup.
...
begin
  ...
  question_hash_value := encode(extensions.digest(normalized_hash_input, 'sha256'), 'hex');
  ...
  assistance_ml := p_payload->'assistance'->'malayalam';
  ...
  if assistance_ml is not null
     and assistance_ml <> 'null'::jsonb
     and assistance_ml <> '{}'::jsonb then
    insert into public.question_metadata (question_id, key, value)
    values (v_question_id, 'assistance_malayalam', assistance_ml)
    on conflict (question_id, key)
    do update set value = excluded.value;
  else
    delete from public.question_metadata
